...
class DoublePendulum(Scene):
    def construct(self):
        ...
        def pendulum_updater(mobj, dt):
            #self.theta1, self.theta2 = self.calc(g, length1, length2, mass1, mass2, v1, v2)
            self.theta1 += self.theta1 * dt
            x1, x2, y1, y2 = self.get_position(length1, length2)
    ...
    def calc(self, g, length1, length2, mass1, mass2, v1, v2):
        numerator = -g * (2*mass1 + mass2) * sin(self.theta1) - mass2 * g * sin(self.theta1 - 2 * self.theta2)
        - 2 * sin(self.theta1 - self.theta2) * mass2 * (v2**2 * length2 + v1**2 * length1 * cos(self.theta1-self.theta2))
        denominator = length1 * (2 * mass1 + mass2 - mass2 * cos(2 * self.theta1 - 2 * self.theta2))
        theta_pp1 = numerator / denominator

        numerator = 2 * sin(self.theta1 - self.theta2) * (v1**2 * length1 * (mass1 + mass2) + g * (mass1 + mass2) 
        * cos(self.theta1) + self.theta2**2 * length2 * mass2 * cos(self.theta1 - self.theta2))
